#!/usr/bin/env python3
"""Launch all algorithm-dimension shards for B21 Track D."""
...
def run_shard(
    *,
    ...
    command = [
        sys.executable,
        "evidence_extension_v1/track_d/run_track_d_shard.py",
        "--mode",
        mode,
        "--algorithm",
        algorithm,
...
def main() -> None:
    ...
    if config["primary_overhead"] and args.workers != 1:
        raise RuntimeError("The registered overhead probe must use exactly one worker.")
    identity = verify_source_identity(require_clean=True)

    result_root = ROOT / args.result_root
    run_root = result_root / args.run_id
    log_root = ROOT / "logs_b21" / "track_d" / args.run_id
    run_root.mkdir(parents=True, exist_ok=True)
    log_root.mkdir(parents=True, exist_ok=True)

    launch_metadata_path = run_root / "launch_metadata.json"
    metadata = {
        "status": "TRACK_D_LAUNCH_STARTED",
        ...
        "primary_overhead": config["primary_overhead"],
        "identity": identity,
    }
            ...
            jobs.append(
                {
                    ...
                    "log_path": log_root
                    / f"{algorithm}_d{dimension}_attempt{attempt:04d}.log",
                    "result_root": args.result_root,
                }
            )

    print(f"Track D mode: {args.mode}")
    ...
    if jobs:
        with ThreadPoolExecutor(
            ...
            for future in as_completed(futures):
                ...
                print(
                    f"{name}: attempt={attempt} returncode={returncode}",
                    flush=True,
                )
                if returncode != 0:
                    failures[name] = returncode
    if failures:
        metadata["status"] = "TRACK_D_LAUNCH_FAILED"
        metadata["failures"] = failures
        launch_metadata_path.write_text(
            json.dumps(metadata, indent=2), encoding="utf-8"
        )
        raise RuntimeError(
            "Track D shard failures: " + json.dumps(failures, sort_keys=True)
        )
    ...
    if len(rows) != config["expected_rows"]:
        raise RuntimeError(
            f"Combined row count mismatch: {len(rows)} != "
    ...
    marker = {
        "status": "TRACK_D_ALL_SHARDS_COMPLETE",
        "run_id": args.run_id,
        "mode": args.mode,
        "rows": len(rows),
        "expected_rows": config["expected_rows"],
    ...
    print("TRACK_D_ALL_SHARDS_COMPLETE")
