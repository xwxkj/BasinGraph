...
case "${MODE}" in
  smoke|confirmatory) ;;
  *)
    echo "Usage: bash evidence_extension_v1/track_b/run_track_b_mac.sh [smoke|confirmatory]" >&2
    exit 2
    ;;
esac
...
VENV_DIR="${B21_TRACK_B_VENV:-${PROJECT_ROOT}/.venv-track-b}"
RESULT_ROOT="results_b21/track_b"
...
python -m pip install -r \
  protocols/evidence_extension_v1/track_b/TRACK_B_REQUIREMENTS_LOCK.txt
...
if [ "${MODE}" = "confirmatory" ]; then
  GATE="${RESULT_ROOT}/TRACK_B_SMOKE_GATE.json"
  if [ ! -f "${GATE}" ]; then
    echo "Track B confirmatory run is locked until the smoke gate exists:" >&2
    echo "  ${PROJECT_ROOT}/${GATE}" >&2
    echo "Run the Track B smoke command first." >&2
    exit 1
  fi
  python - <<PY
import json
...
if gate.get("status") != "TRACK_B_SMOKE_GATE_OK":
    raise SystemExit("Track B smoke gate status is invalid.")
if gate.get("source_commit") != head:
    raise SystemExit(
        "Track B smoke gate belongs to a different source commit. "
        "Rerun smoke on the current frozen runner."
    )
PY
  RUN_ID_FILE="${RESULT_ROOT}/TRACK_B_CONFIRMATORY_RUN_ID.txt"
  if [ -f "${RUN_ID_FILE}" ]; then
    RUN_ID="$(tr -d '[:space:]' < "${RUN_ID_FILE}")"
  else
    RUN_ID="b21_track_b_confirmatory_$(date -u +%Y%m%dT%H%M%SZ)"
    printf '%s\n' "${RUN_ID}" > "${RUN_ID_FILE}"
  fi
  AUTH_FLAG="--authorize-confirmatory"
else
  RUN_ID="b21_track_b_smoke_$(date -u +%Y%m%dT%H%M%SZ)"
  AUTH_FLAG=""
fi

printf '\n=== Track B protocol manifest ===\n'
...
printf '\n=== Track B zero-evaluation preflight ===\n'
python evidence_extension_v1/track_b/preflight_track_b.py \
  --mode "${MODE}" \
  --base-seed 20260808 \
  ${AUTH_FLAG}

printf '\n=== Track B shard execution ===\n'
python evidence_extension_v1/track_b/run_track_b_all.py \
  --mode "${MODE}" \
  --workers "${B21_WORKERS}" \
  --run-id "${RUN_ID}" \
  --base-seed 20260808 \
...
printf '\n=== Track B validation and official cocopp ===\n'
python evidence_extension_v1/track_b/finalize_track_b.py \
  --mode "${MODE}" \
  --run-id "${RUN_ID}" \
  --base-seed 20260808 \
  ${AUTH_FLAG}
...
if report["status"] != "TRACK_B_FINAL_VALIDATION_OK":
    raise SystemExit("Track B smoke gate not satisfied.")
head = subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()
gate = {
    "status": "TRACK_B_SMOKE_GATE_OK",
    "run_id": "${RUN_ID}",
    "source_commit": head,
    "source_identity_sha256": report["identity"]["source_identity_sha256"],
}
Path("${PROJECT_ROOT}/${RESULT_ROOT}/TRACK_B_SMOKE_GATE.json").write_text(
...
DELIVERY_DIR="${DELIVERY_ROOT}/trackB_${MODE}_${TIMESTAMP}"
mkdir -p "${DELIVERY_DIR}"

LIGHT_ZIP="B21_TRACK_B_${MODE}_${TIMESTAMP}.zip"
LIGHT_SHA="${LIGHT_ZIP}.sha256"
OBSERVER_TAR="B21_TRACK_B_${MODE}_observer_${TIMESTAMP}.tar.gz"
...
B21 Track B ${MODE} completed successfully.
...
- ${DELIVERY_DIR}/algorithm_summary.csv
